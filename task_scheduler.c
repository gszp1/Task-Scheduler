...
int main(int argc, char* argv[]) {
    ...
    mqd_t server_msg_queue = mq_open(SERVER_QUEUE_NAME, O_CREAT | O_EXCL | O_RDONLY, 0444, &server_msg_queue_attributes);
    if ((server_msg_queue == -1) && (errno == EEXIST)) {
        ...
        server_msg_queue  = mq_open(SERVER_QUEUE_NAME, O_WRONLY, 0222, &server_msg_queue_attributes);
